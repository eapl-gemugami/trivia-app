...
function jumble(textLen) {
	let generated = ''
	//const randomChar = () => String.fromCharCode(scramble.random(65, 126))
	// Only lowecase characters
	const randomChar = () => String.fromCharCode(scramble.random(97, 122))
	...
	if (el instanceof NodeList) {
		...
		const iterateLetters = () => {
			...
			if (letterIdx++ >= totalChars) {
				clearTimeout(iterateLetters);
			} else {
				setTimeout(iterateLetters, 100)
			}
		};
		return {
